...
- [x] **Lossless Master Album Deduplication**: Evidence-backed deduplication algorithm (`DedupeAlbums`) merging duplicate digital/streaming releases into 1-to-1 master albums, matching on Discogs master ID, normalized title equality (`NormalizeAlbumTitle`), or track title overlap — normalized title equality alone is sufficient evidence, fixing false negatives where duplicate albums had complementary (non-overlapping) tracklists. Merges reassign tracks/`release_versions` to the canonical album without inserting placeholder `release_versions` rows, so the album detail page's Discogs Pressings table only ever shows real pressings. Triggerable via CLI flag (`go run . -dedupe-albums`) or UI Settings dropdown menu.
